...
```{r}
recipe <- recipe(heart_disease ~ ., data = train_data) %>% # Create a recipe object for data preprocessing
...
# model 1
model_1 <- glm(heart_disease == 1 ~ age + sex + trestbps + chol + fbs + thalach, data = train_processed, family = binomial)
summary(model_1)

# model 2
model_2 <- glm(heart_disease ==1  ~ age + sex + trestbps + chol + fbs + thalach + sex*age_group, data = train_processed, family = binomial)
summary(model_2)
```
...
Create Anova table for the models:
```{r}
anova(model_1)
anova(model_2)
```
