...
Load the dataset and view the first rows. In this case we will use the dataset "blabla" which blabla. Metadata can be found in: https://search.r-project.org/CRAN/refmans/kmed/html/heart.html 
```{r}
library("kmed")
data <- heart
head(data)
...
Create model where the dependent variable is having a heart disease and the independent variables are age, sex, Resting blood pressure in mm Hg, Serum cholesterol in mg/dl, Fasting blood sugar more than 120 mg/dl and Maximum heart rate achieved.
```{r}
...
model <- lm(heart_disease ~ age + sex + trestbps + chol + fbs + thalach, data = data)
