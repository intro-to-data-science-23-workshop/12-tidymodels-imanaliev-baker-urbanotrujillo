...
```
 ...
 ## PLEASE INTEGRATE THIS STEPS IN RECEPIE ##
```{r}
...
```{r}
# data$class <- as.factor(data$class) OUR DEPENDENT VAR IS heart_disease
```
...
Set up the enhanced recipe for pre-processing:
```{r}
recipe <- recipe(class ~ ., data = train_data) %>% # Create a recipe object for data preprocessing
  step_impute_median(all_numeric(), -all_outcomes()) %>% # Replace missing values in numeric columns with median values
  step_unknown(all_nominal(), -all_outcomes()) %>%# Handle unknown levels in categorical columns 
  step_dummy(all_nominal(), -all_outcomes()) %>% # Convert categorical variables to dummy/indicator variables
